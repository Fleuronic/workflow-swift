...
extension UpdateChildScreenViewController {
	...
	public func update<ViewController: UIViewController, ScreenType: Screen>(child: ReferenceWritableKeyPath<Self, ViewController>, with screen: ScreenType, in environment: ViewEnvironment, onChange: (ViewController) -> Void = { _ in }) {
		...
		if description.canUpdate(viewController: existing) {
			...
			let new = description.buildViewController() as! ViewController
			
			// We already have a reference to the old view controller above, update the key path to the new one.
			...
			if let parent = old.parent {
				...
				// Begin the transition: Signal the new ViewController will begin moving in, and the old one, out.
				parent.addChild(new)
				...
				// Finish the transition by signaling the ViewController they've fully moved in / out.
